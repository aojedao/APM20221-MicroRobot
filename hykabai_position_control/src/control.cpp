...
int num_slice2 = 4;               // divide a circle into segments

double maxSpeed = 0.08;
double distanceConst = 0.5;
double dt = 0.1, maxT = M_PI, minT = -M_PI, Kp =0.95, Ki = 0.01, Kd = 0.1; //best so far 0.95,0.01,0.1
double dtS = 0.1, maxS = maxSpeed, minS = 0.04, KpS = 0.4, KiS = 0.005, KdS = 0.01; // 0.4,0.005,0.01
...
void displayLane(bool isTrajectoryPushed, Geometry &geometry) {
    ...
    while (path.points.size() <= num_slice2) {
        ...
        slice_index2++;
        p.x = 3 * cos(angle) - 0.5;       //some random circular trajectory, with radius 4, and offset (-0.5, 1, 0)
        p.y = 3 * sin(angle) + 1;
        p.z = 0;
